...
- 결과는 가능하면 `.md` 파일 또는 Markdown 형식으로 받는다.
- 조사 결과가 확정된 뒤에만 필요할 때 기존 문서에 반영한다.
...
### 결과물 형식
- 길고 중요한 분석/기획/작업 결과 → `.md` 파일로 요청
- 기존 문서 보강 → 새 파일 만들지 말고 기존 md 파일 업데이트
- 임시 확인/짧은 조사 → 파일 생성 없이 Markdown 형식 답변
- 표가 많거나 길어질 내용 → 가능하면 md 파일
...
- 긴 결과는 `.md` 파일 또는 Markdown 형식으로 받는다.
- 기존 문서 보강은 새 파일이 아니라 기존 md 파일 업데이트로 지시한다.
- 기존 코드 구현 여부가 중요한 경우에는 먼저 조사하고, 필요할 때만 문서에 반영한다.
...
6. Codex 작업 결과를 md 파일 또는 Markdown 형식으로 받아 검토
7. 다음 항목은 사용자가 다시 선택
